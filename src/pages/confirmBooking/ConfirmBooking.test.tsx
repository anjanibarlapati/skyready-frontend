...
import { flightsReducer, setAlert, clearFlights } from "../../redux/flightsSlice";
...
const renderConfirmBooking = () => {
  return render(
    <Provider store={createMockStore()}>
      <MemoryRouter initialEntries={[{ pathname: "/confirm-booking", state: { flight: mockFlight } }]}>
        <Routes>
...
describe("ConfirmBooking Component", () => {
  ...
  test("network error shows fallback alert", async () => {
    ...
    await waitFor(() => {
      expect(mockDispatch).toHaveBeenCalledWith(
        setAlert({ type: "failure", message: "❌ Network error. Please try again." })
      );
      expect(mockDispatch).toHaveBeenCalledWith(clearFlights());
    });
  });
